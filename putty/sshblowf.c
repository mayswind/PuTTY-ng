...
static void blowfish_key(void *handle, const void *key)
{
    ...
    blowfish_setkey(ctx, (const unsigned char *)key, 16);
}

static void blowfish256_key(void *handle, const void *key)
{
    BlowfishContext *ctx = (BlowfishContext *)handle;
    blowfish_setkey(ctx, (const unsigned char *)key, 32);
}
...
{
    BlowfishContext *ctx = (BlowfishContext *)handle;
    blowfish_setkey(ctx, (const unsigned char *)key, SSH_SESSION_KEY_LENGTH);
    ctx->iv0 = 0;
    ctx->iv1 = 0;
