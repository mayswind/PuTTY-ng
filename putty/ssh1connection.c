...
static void ssh1_connection_free(PacketProtocolLayer *ppl)
{
    ...
    while ((c = (struct ssh1_channel *)delpos234(s->channels, 0)) != NULL)
        ssh1_channel_free(c);
        ...
        switch (pktin->type) {
          ...
          case SSH1_SMSG_X11_OPEN:
            ...
            if (!s->X11_fwd_enabled) {
                ...
                ssh1_channel_init(c);
                c->remoteid = remid;
                c->chan = x11_new_channel(s->ppl.frontend, s->x11authtree, &c->sc,
                                          NULL, -1, FALSE);
                c->remoteid = remid;
                c->halfopen = FALSE;
          ...
          case SSH1_MSG_PORT_OPEN:
            ...
            pf.dhost = mkstr(host);
            pf.dport = port;
            pfp = (struct ssh_rportfwd *)find234(s->rportfwds, &pf, NULL);
          ...
          case SSH1_MSG_CHANNEL_CLOSE_CONFIRMATION:
            ...
            c = (struct ssh1_channel *)find234(s->channels, &localid, ssh1_channelfind);
          ...
          case SSH1_SMSG_STDERR_DATA:
            ...
            if (!get_err(pktin)) {
                int bufsize = from_backend(
                    (Frontend *)s->ppl.frontend, pktin->type == SSH1_SMSG_STDERR_DATA,
                    data.ptr, data.len);
...
static void ssh1_connection_process_queue(PacketProtocolLayer *ppl)
{
	...
	put_uint32(pktout, 0); /* height in pixels */
        write_ttymodes_to_packet_from_conf(
            BinarySink_UPCAST(pktout), (Frontend *)s->ppl.frontend, s->conf,
            1, s->ospeed, s->ispeed);
        pq_push(s->ppl.out_pq, pktout);
        crMaybeWaitUntilV((pktin = ssh1_connection_pop(s)) != NULL);
	if (pktin->type == SSH1_SMSG_SUCCESS) {
...
static void ssh1_throttle_all_channels(ConnectionLayer *cl, int throttled)
{
    ...
    for (i = 0; NULL != (c = (struct ssh1_channel *)index234(s->channels, i)); i++)
