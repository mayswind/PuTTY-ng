                        ...
                        void *logctx, pageant_logfn_t logfn)
{
	 ...
	 */
	{
	    ...
	    int i;
            ...
            session_id = marshal_get_data(msg, 16);
	    ...
	    marshal_put_data(&md5c, session_id.ptr, session_id.len);
	    MD5Final(response_md5, &md5c);

	    put_byte(bs, SSH1_AGENT_RSA_RESPONSE);
	    marshal_put_data(bs, response_md5, 16);
	 ...
	 */
	{
	    struct ssh2_userkey *key;
            ...
            key = (ssh2_userkey *)find234(ssh2keys, &keyblob, cmpkeys_ssh2_asymm);
	    if (!key) {
                pageant_failure_msg(bs, "key not found", logctx, logfn);
                return;
            }
	 ...
	 */
	{
	    struct ssh2_userkey *key;
            ...
            key = (ssh2_userkey *)find234(ssh2keys, &blob, cmpkeys_ssh2_asymm);
	    if (!key) {
                pageant_failure_msg(bs, "key not found", logctx, logfn);
                return;
            }
                      ...
                      char **retstr)
{
    ...
    for (i = 0; i < nkeys; i++) {
        ...
        cbkey.blob = strbuf_new();
        marshal_put_data(cbkey.blob, pubblob.ptr, pubblob.len);
...
int pageant_delete_key(struct pageant_pubkey *key, char **retstr)
{
    ...
    if (key->ssh_version == 1) {
        ...
        marshal_put_data(request, key->blob->s, key->blob->len);
    } else {
...
struct pageant_pubkey *pageant_pubkey_copy(struct pageant_pubkey *key)
{
    ...
    ret->blob = strbuf_new();
    marshal_put_data(ret->blob, key->blob->s, key->blob->len);
