...
static void ssh1_login_process_queue(PacketProtocolLayer *ppl)
{
    ...
    {
        ptrlen pl = marshal_get_data(pktin, 8);
     ...
     */
    {
        ...
        if (s->dlgret == 0) {          /* did not match */
            ...
            s->dlgret = verify_ssh_host_key(
                (Frontend *)s->ppl.frontend, s->savedhost, s->savedport,
	...
	for (i = 0; !cipher_chosen && i < CIPHER_MAX; i++) {
        ...
        if (warn) {
            s->dlgret = askalg((Frontend *)s->ppl.frontend, "cipher", cipher_string,
            ...
            if (get_byte(s->asrc) == SSH1_AGENT_RSA_IDENTITIES_ANSWER) {
                ...
                for (s->keyi = 0; s->keyi < s->nkeys; s->keyi++) {
...

                    {
                        ...
                        ret = (const char *)s->agent_response.ptr;
                        if (ret) {
