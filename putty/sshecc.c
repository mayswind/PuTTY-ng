...
static int BinarySource_get_point(BinarySource *src, struct ec_point *point)
{
    ...
    return decodepoint((const char *)str.ptr, str.len, point);
}
                                         ...
                                         BinarySource *src)
{
    ...
    if (!decodepoint_ed((const char *)p.ptr, p.len, &ec->publicKey))
    {
        ecdsa_freekey(&ec->sshk);
        return NULL;
    }
...
static void ed25519_openssh_blob(ssh_key *key, BinarySink *bs)
{
    ...
    marshal_put_data(bs, pub->s, pub->len);
     ...
     * reason requires */
    marshal_put_data(bs, pub->s + 4, pub->len - 4);
...
static int ecdsa_verify(ssh_key *key, ptrlen sig, ptrlen data)
{
    ...
    if (ec->publicKey.curve->type == EC_EDWARDS) {
        ...
        if (!decodepoint((const char *)sigstr.ptr, pointlen, r)) {
            ec_point_free(r);
            return 0;
        }
        ...
        /* Get the hash of the encoded value of R + encoded value of pk + message */
        {
             ...
             * the signature) */
            marshal_put_data(&hs, sigstr.ptr, pointlen);
            ...
            marshal_put_data(&hs, data.ptr, data.len);
        ...
        marshal_put_data(extra->hash->sink(hashctx), data.ptr, data.len);
                       ...
                       BinarySink *bs)
{
    ...
    if (ec->publicKey.curve->type == EC_EDWARDS) {
         ...
         * S = (r + H(encodepoint(R) + encodepoint(pk) + m) * a) % l */
        {
            ...
            SHA512_Init(&hs);
            marshal_put_data(&hs, hash+(ec->publicKey.curve->fieldBits / 8),
                     ((ec->publicKey.curve->fieldBits / 4) -
                      (ec->publicKey.curve->fieldBits / 8)));
            marshal_put_data(&hs, data, datalen);
            SHA512_Final(&hs, hash);
        ...
        assert(digestLen <= sizeof(digest));
        hashctx = extra->hash->init();
        marshal_put_data(extra->hash->sink(hashctx), data, datalen);
        extra->hash->final(hashctx, digest);
                          ...
                          const void *remoteKey, int remoteKeyLen)
{
    ...
    if (ec->publicKey.curve->type == EC_WEIERSTRASS) {
        ...
        remote.infinity = 0;
        if (!decodepoint((const char *)remoteKey, remoteKeyLen, &remote)) {
            return NULL;
        }
    } else {
