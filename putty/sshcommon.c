...
{
    logevent((Frontend *)frontend, message);
    sfree(message);
}

void ssh_ppl_replace(PacketProtocolLayer *old_ppl, PacketProtocolLayer *new_ppl)
{
    new_ppl->bpp = old_ppl->bpp;
    ssh_ppl_setup_queues(new_ppl, old_ppl->in_pq, old_ppl->out_pq);
    new_ppl->selfptr = old_ppl->selfptr;
    new_ppl->user_input = old_ppl->user_input;
    new_ppl->frontend = old_ppl->frontend;
    new_ppl->ssh = old_ppl->ssh;

    *new_ppl->selfptr = new_ppl;
    ssh_ppl_free(old_ppl);
     ...
     * straight away. */
    queue_idempotent_callback(&new_ppl->ic_process_queue);
}
...
void ssh_ppl_user_output_string_and_free(PacketProtocolLayer *ppl, char *text)
{
     ...
     * flag set. */
    from_backend((Frontend *)ppl->frontend, TRUE, text, strlen(text));
    sfree(text);
}
