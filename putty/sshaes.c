...
{
    aes_setup(ctx, (const unsigned char *)key, 16);
}

void aes192_key(AESContext *ctx, const void *key)
{
    aes_setup(ctx, (const unsigned char *)key, 24);
}

void aes256_key(AESContext *ctx, const void *key)
{
    aes_setup(ctx, (const unsigned char *)key, 32);
}
...
{
    aes_encrypt_cbc((unsigned char *)blk, len, ctx);
}

void aes_ssh2_decrypt_blk(AESContext *ctx, void *blk, int len)
{
    aes_decrypt_cbc((unsigned char *)blk, len, ctx);
}

void aes_ssh2_sdctr(AESContext *ctx, void *blk, int len)
{
    aes_sdctr((unsigned char *)blk, len, ctx);
}
...
ssh2_cipher *aes_ssh2_newobj(const struct ssh2_cipheralg *alg)
{
...
{
    struct aes_ssh2_ctx *ctx = FROMFIELD(cipher, struct aes_ssh2_ctx, vt);
    aes_setup(&ctx->context, (const unsigned char *)key, ctx->vt->padded_keybytes);
}

static void aes_ssh2_encrypt(ssh2_cipher *cipher, void *blk, int len)
{
    struct aes_ssh2_ctx *ctx = FROMFIELD(cipher, struct aes_ssh2_ctx, vt);
    aes_encrypt_cbc((unsigned char *)blk, len, &ctx->context);
}

static void aes_ssh2_decrypt(ssh2_cipher *cipher, void *blk, int len)
{
    struct aes_ssh2_ctx *ctx = FROMFIELD(cipher, struct aes_ssh2_ctx, vt);
    aes_decrypt_cbc((unsigned char *)blk, len, &ctx->context);
}

static void aes_ssh2_sdctr_method(ssh2_cipher *cipher, void *blk, int len)
{
    struct aes_ssh2_ctx *ctx = FROMFIELD(cipher, struct aes_ssh2_ctx, vt);
    aes_sdctr((unsigned char *)blk, len, &ctx->context);
}

static const struct ssh2_cipheralg ssh_aes128_ctr = {
    aes_ssh2_newobj, aes_ssh2_free, aes_ssh2_setiv, aes_ssh2_setkey,
