                       ...
                       Plug *plug, Conf *conf)
{
	...
	BasePlugContainer *plugcontainer = FROMFIELD(plug, struct BasePlugContainer, plugvt);

	ret = snew(ProxySocket);
	ret->frontend = plugcontainer->frontend;
          ...
          case ADDRTYPE_IPV4:
            {
                ...
                marshal_put_data(command, addr, 4);
                break;
            }
          case ADDRTYPE_NAME:
...
int proxy_socks5_negotiate (ProxySocket *p, int change)
{
	...
	if (p->state == 2) {
		...
		sk_addrcopy(p->remote_addr, (char *)strbuf_append(command, 4));
                break;
              case ADDRTYPE_IPV6:
		put_byte(command, 4);  /* IPv6 */
		sk_addrcopy(p->remote_addr, (char *)strbuf_append(command, 16));
                break;
              case ADDRTYPE_NAME:
                {
                    char hostname[512];
