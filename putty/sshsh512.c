...
static ssh_hash *sha512_newobj(const struct ssh_hashalg *alg)
{
...
static ssh_hash *sha512_copy(ssh_hash *hashold)
{
    ...
    ssh_hash *hashnew = sha512_newobj(hashold->vt);
...
const struct ssh_hashalg ssh_sha512 = {
    sha512_newobj, sha512_copy, sha512_final, sha512_free, 64, "SHA-512"
};

static ssh_hash *sha384_newobj(const struct ssh_hashalg *alg)
{
    struct sha512_hash *h = snew(struct sha512_hash);
    SHA384_Init(&h->state);
    h->hash.vt = alg;
...
const struct ssh_hashalg ssh_sha384 = {
    sha384_newobj, sha512_copy, sha384_final, sha512_free, 48, "SHA-384"
};
...
int main(void) {
	...
	if (tests[i].teststring) {
		...
		marshal_put_data(&s, "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", 40);
