...
void MD5Simple(void const *p, unsigned len, unsigned char output[16])
{
    ...
    MD5Init(&s);
    marshal_put_data(&s, (unsigned char const *)p, len);
    MD5Final(output, &s);
    smemclr(&s, sizeof(s));
}
...
static ssh2_mac *hmacmd5_ssh2_newobj(const struct ssh2_macalg *alg,
                                  ssh2_cipher *cipher)
{
...
void hmacmd5_key(struct hmacmd5_context *ctx, void const *keyv, int len)
{
	...
	foo[i] ^= key[i];
    MD5Init(&ctx->md5[0]);
    marshal_put_data(&ctx->md5[0], foo, 64);

    memset(foo, 0x5C, 64);
    for (i = 0; i < len && i < 64; i++)
	foo[i] ^= key[i];
    MD5Init(&ctx->md5[1]);
    marshal_put_data(&ctx->md5[1], foo, 64);
...
static void hmacmd5_genresult(ssh2_mac *mac, unsigned char *hmac)
{
    ...
    BinarySink_COPIED(&s);
    marshal_put_data(&s, intermediate, 16);
    MD5Final(hmac, &s);
                     ...
                     const void *blk, int len, unsigned char *hmac)
{
    ...
    marshal_put_data(&ctx->mac, blk, len);
    return ssh2_mac_genresult(&ctx->mac, hmac);
}

const struct ssh2_macalg ssh_hmac_md5 = {
    hmacmd5_ssh2_newobj, hmacmd5_ssh2_free, hmacmd5_ssh2_setkey,
