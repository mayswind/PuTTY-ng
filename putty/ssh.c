 ...
 */
typedef enum { THROTTLED, UNTHROTTLING, UNTHROTTLED } SSH_CHANNEL_throttle_state;
struct ssh_channel {
 ...
 * it.
 */
typedef enum { DESTROY, KEEP, CREATE } SSH_PORTFWD_STATUS;
struct ssh_portfwd {
    int status;
    int type;
    unsigned sport, dport;
...
#define logevent(s) logevent(ssh->frontend, (const char*)s)
			     ...
			     const void *sigblob, int sigblob_len)
{
	...
	if (mod_mp.len != sig_mp.len) {
	    ...
	    marshal_put_data(substr, sigblob, sig_prefix_len);
	    put_uint32(substr, mod_mp.len);
	    put_padding(substr, mod_mp.len - sig_mp.len, 0);
	    marshal_put_data(substr, sig_mp.ptr, sig_mp.len);
...
static void do_ssh1_login(void *vctx)
{
    ...
    marshal_put_data(&md5c, s->cookie, 8);
    ...
    marshal_put_data(pkt, s->cookie, 8);
    put_uint16(pkt, s->len * 8);
    marshal_put_data(pkt, s->rsabuf, s->len);
    ...
    if ((ssh->username = get_remote_username(ssh->conf)) == NULL) {
        ...
        s->userpass_ret = get_userpass_input(ssh->frontend, s->cur_prompt, NULL);
        while (1) {
            while (s->userpass_ret < 0 &&
                   bufchain_size(&ssh->user_input) > 0)
                s->userpass_ret = get_userpass_input(
                    ssh->frontend, s->cur_prompt, &ssh->user_input);
	...
	if (conf_get_int(ssh->conf, CONF_tryagent) && agent_exists() && !s->tried_agent) {
		...
		for (s->keyi = 0; s->keyi < s->nkeys; s->keyi++) {
			...
			marshal_put_data(agentreq, s->session_id, 16);
	...
	if (s->publickey_blob && s->privatekey_available &&
...

		{
		    ...
		    MD5Init(&md5c);
		    marshal_put_data(&md5c, buffer, 32);
		    marshal_put_data(&md5c, s->session_id, 16);
		    MD5Final(buffer, &md5c);

                    pkt = ssh_bpp_new_pktout(
                        ssh->bpp, SSH1_CMSG_AUTH_RSA_RESPONSE);
                    marshal_put_data(pkt, buffer, 16);
                    ssh_pkt_write(ssh, pkt);
...
int sshfwd_write(struct ssh_channel *c, const void *buf, int len)
{
    ...
    return ssh_send_channel_data(c, (const char *)buf, len);
}
                                 ...
                                 ssh_sharing_connstate *share_ctx)
{
    ...
    pf.shost = dupstr(shost);
    pf.sport = sport;
    realpf = (struct ssh_rportfwd *)del234(ssh->rportfwds, &pf);
    assert(realpf);
...
static void ssh_sharing_global_request_response(Ssh ssh, PktIn *pktin,
                                                void *ctx)
{
    share_got_pkt_from_server((ssh_sharing_connstate *) ctx, pktin->type,
...
static void ssh1_smsg_x11_open(Ssh ssh, PktIn *pktin)
{
	...
	ssh_channel_init(c);
        c->chan = x11_new_channel(ssh->frontend, ssh->x11authtree, c, NULL, -1, FALSE);
        c->remoteid = remoteid;
        c->halfopen = FALSE;
...
static void ssh1_msg_port_open(Ssh ssh, PktIn *pktin)
{
	...
	logeventf(ssh, "Received remote port open request for %s:%d",
		  pf.dhost, port);
        err = pfd_connect(ssh->frontend, &c->chan, pf.dhost, port,
...
static void add_to_commasep(strbuf *buf, const char *data)
{
    if (buf->len > 0)
	put_byte(buf, ',');
    marshal_put_data(buf, data, strlen(data));
}
                       ...
                       char chr, int keylen)
{
    ...
    out->len = 0;
    key = (unsigned char *)strbuf_append(out, keylen_padded);
	...
	put_mp_ssh2(bs, K);
    marshal_put_data(bs, H, h->hlen);
    put_byte(bs, chr);
    marshal_put_data(bs, ssh->v2_session_id, ssh->v2_session_id_len);
    h->final(s, key);
    ...
    if (keylen_padded > h->hlen) {
        ...
        for (offset = h->hlen; offset < keylen_padded; offset += h->hlen) {
            marshal_put_data(bs, key + offset - h->hlen, h->hlen);
            s2 = h->copy(s);
...
static void ssh_cleanup_transient_hostkey_store(Ssh ssh)
{
    ...
    while ((ent = (struct ssh_transient_hostkey_cache_entry *)delpos234(ssh->transient_hostkey_cache, 0)) != NULL) {
...
static void ssh_store_transient_hostkey(Ssh ssh, ssh_key *key)
{
    ...
    if ((ent = (struct ssh_transient_hostkey_cache_entry *)find234(ssh->transient_hostkey_cache, (void *)ssh_key_alg(key),
    ...
    retd = (struct ssh_transient_hostkey_cache_entry *)add234(ssh->transient_hostkey_cache, ent);
    assert(retd == ent);
}
...
{
    struct ssh_transient_hostkey_cache_entry *ent =
        (struct ssh_transient_hostkey_cache_entry *)find234(ssh->transient_hostkey_cache, (void *)alg,
...
static void do_ssh2_transport(void *vctx)
{
  ...
  begin_key_exchange:
	...
	marshal_get_data(pktin, 16);           /* skip garbage cookie */
	...
	for (i = 0; i < NKEXLIST; i++) {
		...
		if (in_commasep_string(alg->name, (const char *)str.ptr, str.len)) {
    ...
    if (ssh->kex->main_type == KEXTYPE_DH) {
        ...
        do {
            ...
            if (!s->init_token_sent) {
                ...
                put_string(s->pktout,
                           (const char*)s->gss_sndtok.value, s->gss_sndtok.length);
                put_mp_ssh2(s->pktout, s->e);
                ssh_pkt_write(ssh, s->pktout);
	 ...
	 */
	{
        ...
        logevent(s->fingerprint);
        sfree(s->fingerprint);
        gStorage->store_host_key(ssh->savedhost, ssh->savedport,
...
static void ssh2_msg_channel_open(Ssh ssh, PktIn *pktin)
{
	...
	if (!ssh->X11_fwd_enabled && !ssh->connshare)
	    error = "X11 forwarding is not enabled";
            ...
            c->chan = x11_new_channel(ssh->frontend, ssh->x11authtree, c, addrstr, peerport,
                                      ssh->connshare != NULL);
            logevent("Opened X11 forward channel");
	}
	...
	if (realpf == NULL) {
	    error = "Remote port is not recognised";
            ...
            err = pfd_connect(ssh->frontend, &c->chan, realpf->dhost, realpf->dport,
...
static void do_ssh2_userauth(void *vctx)
{
	...
	while (1) {
		...
		s->can_pubkey =
		    in_commasep_string("publickey", (const char *)methods.ptr, methods.len);
		s->can_passwd =
		    in_commasep_string("password", (const char *)methods.ptr, methods.len);
		s->can_keyb_inter =
                    conf_get_int(ssh->conf, CONF_try_ki_auth) &&
		    in_commasep_string("keyboard-interactive",
                                       (const char *)methods.ptr, methods.len);
#ifndef NO_GSSAPI
                s->can_gssapi =
		...
		if (pktin->type != SSH2_MSG_USERAUTH_PK_OK) {
		    ...
		    if (ssh->remote_bugs & BUG_SSH2_PK_SESSIONID) {
                        marshal_put_data(sigdata, ssh->v2_session_id,
                                 ssh->v2_session_id_len);
                    } else {
                        put_string(sigdata, ssh->v2_session_id,
                                   ssh->v2_session_id_len);
                    }
		    marshal_put_data(sigdata, s->pktout->data + 5,
                             s->pktout->length - 5);
		...
		while (!key) {
			...
			add_prompt(s->cur_prompt,
				   ...
				   FALSE);
			s->userpass_ret = get_userpass_input(
                            ssh->frontend, s->cur_prompt, NULL);
			while (1) {
                            while (s->userpass_ret < 0 &&
                                   bufchain_size(&ssh->user_input) > 0)
                                s->userpass_ret = get_userpass_input(
		...
		marshal_put_data(s->pktout, s->gss_buf.value, s->gss_buf.length);
		ssh2_pkt_send(ssh, s->pktout);
		crMaybeWaitUntilV((pktin = pq_pop(&ssh->pq_ssh2_userauth)) != NULL);
...
static void do_ssh2_connection(void *vctx)
{
	...
	if ((pktin = pq_pop(&ssh->pq_ssh2_connection)) != NULL) {
	     ...
	     */
            ...
            int len;
            bufchain_prefix(&ssh->user_input, &data, &len);
	    ssh_send_channel_data(ssh->mainchan, (const char *)data, len);
            bufchain_consume(&ssh->user_input, len);
	}
	crReturnV;
    }
...
static void ssh_free(Backend *be)
{
    ...
    while ((auth = (struct X11FakeAuth*)delpos234(ssh->x11authtree, 0)) != NULL)
        x11_free_fake_auth(auth);
    if (ssh->bpp)
                                     ...
                                     const char *additional_log_text)
{
    ...
    marshal_put_data(pkt, data, datalen);
