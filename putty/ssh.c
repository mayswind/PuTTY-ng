...
#define crStateP(t, v)				\
    struct t *s; 				\
    if (!(v)) { (v) = snew(struct t); s = (struct t *)(v);s->crLine = 0; }	\
    s = (struct t *)(v);
    ...
    Ssh ssh, const struct ssh_signkey *alg, void *key)
{
    ...
    if ((ent = (struct ssh_transient_hostkey_cache_entry *)find234(ssh->transient_hostkey_cache, (void *)alg,
			     ...
			     struct Packet *pktin)
{
  ...
  begin_key_exchange:
    ...
    if (s->need_gss_transient_hostkey) {
         ...
         * state is "fresh".
         */
        if (!vin || strcmp((const char *)vin, GSS_UPDATE_REKEY_REASON) != 0)
	...
	if (inlen == -2) {
	    ...
	    s->userauth_succeeded = TRUE;
            if (s->pending_compression) {
                in = (const unsigned char*)"enabling delayed compression";
            } else if (s->need_gss_transient_hostkey) {
                in = (const unsigned char*)"populating transient host key cache";
            } else {
...
static int ssh2_try_send(struct ssh_channel *c)
{
	...
	ssh2_pkt_addstring_data(pktout, (const char*)data, len);
	ssh2_pkt_send(ssh, pktout);
			     ...
			     struct Packet *pktin)
{
	...
	while (1) {
		...
		do {
			...
			ssh_pkt_addstring_start(s->pktout);
                        ssh_pkt_addstring_data(s->pktout, (const char*)s->gss_sndtok.value,
                                               s->gss_sndtok.length);
			ssh2_pkt_send(ssh, s->pktout);
                        ssh->gsslib->free_tok(ssh->gsslib, &s->gss_sndtok);
		    }
