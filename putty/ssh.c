...
static void ssh_agentf_callback(void *cv, void *reply, int replylen)
{
    ...
    if (ssh->version == 2) {
	ssh2_add_channel_data(c, (const char*)sentreply, replylen);
	ssh2_try_send(c);
    } else {
			     ...
			     struct Packet *pktin)
{
	...
	while (1) {
		...
		do {
			...
			ssh_pkt_addstring_data(s->pktout,(const char*)s->gss_sndtok.value,s->gss_sndtok.length);
			ssh2_pkt_send(ssh, s->pktout);
		...
		ssh_pkt_addstring_start(s->pktout);
		ssh_pkt_addstring_data(s->pktout, (const char*)mic.value, mic.length);
		ssh2_pkt_send(ssh, s->pktout);
