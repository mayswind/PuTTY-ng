...
{
    const unsigned char *addr = (const unsigned char *)ipv6.ptr;
    assert(ipv6.len == 16);
...
static void pfd_receive(Plug plug, int urgent, char *data, int len)
{
    ...
    if (pf->socks_state != SOCKS_NONE) {
         ...
         */
        marshal_put_data(pf->socksbuf, data, len);
        ...
        marshal_get_data(src, pf->socksbuf_consumed);
                ...
                if (socks_version == 5 && message_type == 1 &&
                    ...
                    switch (get_byte(src)) {
                      case 1:              /* IPv4 */
                        ...
                        break;
                      case 4:              /* IPv6 */
                        pf->hostname = ipv6_to_string(marshal_get_data(src, 16));
                        break;
 ...
 */
static char *pfl_listen(void* frontend, char *desthost, int destport, char *srcaddr,
...
typedef enum { DESTROY, KEEP, CREATE };
...
{
    PortFwdRecord *realpfr = (PortFwdRecord *)del234(mgr->forwardings, pfr);
    if (realpfr == pfr)
        pfr_free(pfr);
}
...
void portfwdmgr_close_all(PortFwdManager *mgr)
{
    ...
    while ((pfr = (PortFwdRecord *)delpos234(mgr->forwardings, 0)) != NULL)
        pfr_free(pfr);
}
...
void portfwdmgr_config(PortFwdManager *mgr, Conf *conf)
{
     ...
     * they were before.
     */
    for (i = 0; (pfr = (PortFwdRecord *)index234(mgr->forwardings, i)) != NULL; i++)
        pfr->status = DESTROY;
    ...
    for (val = conf_get_str_strs(conf, CONF_portfwd, NULL, &key);
        ...
        if (sport && dport) {
            ...
            PortFwdRecord *existing = (PortFwdRecord *)add234(mgr->forwardings, pfr);
            if (existing != pfr) {
    ...
    for (i = 0; (pfr = (PortFwdRecord *)index234(mgr->forwardings, i)) != NULL; i++) {
        if (pfr->status == CREATE) {
            ...
            if (pfr->type == 'L') {
				char *err = pfl_listen(mgr->cl->frontend, pfr->daddr, pfr->dport,
                                       pfr->saddr, pfr->sport,
				...
				char *err = pfl_listen(mgr->cl->frontend, NULL, -1, pfr->saddr, pfr->sport,
                                       mgr->cl, conf, &pfr->local,
                                       pfr->addressfamily);
