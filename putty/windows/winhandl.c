 ...
 */
typedef enum { EOF_NO, EOF_PENDING, EOF_SENT } handle_output_Outgoingeof;
struct handle_output {
    ...
    handle_output_Outgoingeof outgoingeof;
...
HANDLE *handle_get_events(int *nevents)
{
    ...
    n = size = 0;
    if (handles_by_evtomain) {
	for (i = 0; (h = (struct handle *)index234(handles_by_evtomain, i)) != NULL; i++) {
	    if (h->u.g.busy) {
		if (n >= size) {
		    size += 32;
...
void handle_got_event(HANDLE event)
{
    ...
    h = (struct handle *)find234(handles_by_evtomain, &event, handle_find_evtomain);
    if (!h) {
	/*
