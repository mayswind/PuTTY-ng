...
static ber_item BinarySource_get_ber(BinarySource *src)
{
    ...
    toret.data = marshal_get_data(src, length);
    return toret;
}
    ...
    BinarySink *bs, const void *bytesv, int nbytes)
{
    ...
    marshal_put_data(bs, bytes, nbytes);
}
	    ...
	    *p++ = '\0';
	    ...
	    while (isbase64(*p)) {
                ...
                if (base64_chars == 4) {
                    ...
                    marshal_put_data(ret->keyblob, out, len);
                                      ...
                                      const char **errmsg_p)
{
    ...
    if (key->encrypted) {
        ...
        MD5Init(&md5c);
        marshal_put_data(&md5c, passphrase, strlen(passphrase));
        marshal_put_data(&md5c, key->iv, 8);
        MD5Final(keybuf, &md5c);

        MD5Init(&md5c);
        marshal_put_data(&md5c, keybuf, 16);
        marshal_put_data(&md5c, passphrase, strlen(passphrase));
        marshal_put_data(&md5c, key->iv, 8);
        ...
        for (i = 0; i < num_integers; i++) {
            ...
            if (i == 0) {
                ...
                if (i == 1) {
                    ...
                    modptr = (const char *)integer.data.ptr;
                      ...
                      char *passphrase)
{
    ...
    if (ssh_key_alg(key->key) == &ssh_rsa ||
        ...
        for (i = 0; i < nnumbers; i++) {
            ...
            marshal_put_data(seq, numbers[i].ptr, numbers[i].len);
        }
        put_ber_id_len(outblob, 16, seq->len, ASN1_CONSTRUCTED);
        marshal_put_data(outblob, seq->s, seq->len);
        strbuf_free(seq);
        ...
        put_byte(sub, 0);
        marshal_put_data(sub, pubblob->s+39, 1 + pointlen);
                                      ...
                                      const char **errmsg_p)
{
        ...
        switch (key->kdf) {
          ...
          case ON_K_BCRYPT:
            openssh_bcrypt(passphrase,
                           (const unsigned char *)key->kdfopts.bcrypt.salt.ptr,
        ...
        switch (key->cipher) {
          ...
          case ON_E_AES256CTR:
            ...
            }
            {
                 ...
                 * the const from key->_private being a ptrlen */
                if (key->cipher == ON_E_AES256CBC) {
                    aes_ssh2_decrypt_blk(ctx, (char *)key->_private.ptr,
                                         key->_private.len);
                }
                else {
                    aes_ssh2_sdctr(ctx, (char *)key->_private.ptr,
                                   key->_private.len);
                }
                aes_free_context(ctx);
            }
     ...
     */
    BinarySource_BARE_INIT(src, key->_private.ptr, key->_private.len);
                      ...
                      char *passphrase)
{
    ...
    /* Private section. */
    {
        ...
        marshal_put_data(cpblob, privblob->s, privblob->len);
...
static ptrlen BinarySource_get_mp_sshcom_as_string(BinarySource *src)
{
    ...
    return marshal_get_data(src, (bits + 7) / 8);
}
