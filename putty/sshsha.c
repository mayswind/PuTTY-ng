...
static ssh_hash *sha1_newobj(const struct ssh_hashalg *alg)
{
...
static ssh_hash *sha1_copy(ssh_hash *hashold)
{
    ...
    ssh_hash *hashnew = sha1_newobj(hashold->vt);
...
const struct ssh_hashalg ssh_sha1 = {
    sha1_newobj, sha1_copy, sha1_final, sha1_free, 20, "SHA-1"
};
...
static ssh2_mac *hmacsha1_newobj(
                              ...
                              const unsigned char *key, int len)
{
	...
	foo[i] ^= key[i];
    SHA_Init(&keys[0]);
    marshal_put_data(&keys[0], foo, 64);

    memset(foo, 0x5C, 64);
    for (i = 0; i < len && i < 64; i++)
	foo[i] ^= key[i];
    SHA_Init(&keys[1]);
    marshal_put_data(&keys[1], foo, 64);
...
static void hmacsha1_key(ssh2_mac *mac, const void *key)
{
     ...
     * a shorter key */
    sha1_key_internal(ctx->sha, (const unsigned char *)key, ctx->mac.vt->keylen);
}
...
static void hmacsha1_genresult(ssh2_mac *mac, unsigned char *hmac)
{
    ...
    BinarySink_COPIED(&s);
    marshal_put_data(&s, intermediate, 20);
    SHA_Final(&s, intermediate);
...
void hmac_sha1_simple(const void *key, int keylen,
    ...
    marshal_put_data(&states[0], data, datalen);
    SHA_Final(&states[0], intermediate);

    marshal_put_data(&states[1], intermediate, 20);
    SHA_Final(&states[1], output);
}

const struct ssh2_macalg ssh_hmac_sha1 = {
    hmacsha1_newobj, hmacsha1_free, hmacsha1_key,
