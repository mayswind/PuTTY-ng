...
static ssh2_mac *poly_ssh2_newobj(
...
static const struct ssh2_macalg ssh2_poly1305 = {
    poly_ssh2_newobj, poly_ssh2_free, poly_setkey,
...
{
    struct ccp_context *ctx = FROMFIELD(cipher, struct ccp_context, cvt);
    chacha20_encrypt(&ctx->b_cipher, (unsigned char *)blk, len);
}

static void ccp_decrypt(ssh2_cipher *cipher, void *blk, int len)
{
    struct ccp_context *ctx = FROMFIELD(cipher, struct ccp_context, cvt);
    chacha20_decrypt(&ctx->b_cipher, (unsigned char *)blk, len);
}
                               ...
                               unsigned long seq)
{
    ...
    chacha20_encrypt(&ctx->a_cipher, (unsigned char *)blk, len);
}
                               ...
                               unsigned long seq)
{
    struct ccp_context *ctx = FROMFIELD(cipher, struct ccp_context, cvt);
    ccp_length_op(ctx, blk, len, seq);
    chacha20_decrypt(&ctx->a_cipher, (unsigned char *)blk, len);
}
