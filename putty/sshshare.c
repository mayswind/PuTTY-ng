                                      ...
                                      struct share_channel *chan)
{
    ...
    if (type == SSH2_MSG_CHANNEL_DATA) {
        ...
        do {
            ...
            marshal_put_data(packet, data.ptr, this_len);
        ...
        put_uint32(packet, 0);     /* placeholder for length field */
        put_byte(packet, type);
        marshal_put_data(packet, pkt, pktlen);
        PUT_32BIT(packet->s, packet->len-4);
        sk_write(cs->sock, packet->s, packet->len);
        strbuf_free(packet);
    }
    ...
    struct share_xchannel *xc, int type, const void *data, int len)
{
    ...
    msg->data = (unsigned char *)snew_plus_get_aux(msg);
    msg->datalen = len;
    msg->type = type;
                             ...
                             const void *initial_data, int initial_len)
{
    ...
    marshal_put_data(packet, greeting, greeting_len);
    marshal_put_data(packet, initial_data, initial_len);
    sfree(greeting);
                                  ...
                                  accept_fn_t constructor, accept_ctx_t ctx)
{
    ...
    cs->frontend = sharestate->frontend;
    ...
    Plug sshplug, ssh_sharing_state **state)
{
    ...
    sharestate->frontend = cl->frontend;
