    ...
    Bignum K, unsigned char *H, char chr, int keylen)
{
    ...
    if (!(s->ppl.remote_bugs & BUG_SSH2_DERIVEKEY))
        put_mp_ssh2(h, K);
    marshal_put_data(h, H, hlen);
    put_byte(h, chr);
    marshal_put_data(h, s->session_id, s->session_id_len);
    ...
    if (keylen_padded > hlen) {
        ...
        for (offset = hlen; offset < keylen_padded; offset += hlen) {
            marshal_put_data(h, key + offset - hlen, hlen);
...
static void ssh2_transport_process_queue(PacketProtocolLayer *ppl)
{
  ...
  begin_key_exchange:
     ...
     */
    {
        ...
        if (s->warn_kex) {
            s->dlgret = askalg((Frontend *)s->ppl.frontend, "key-exchange algorithm",
                               s->kex_alg->name,
        ...
        if (s->warn_hk) {
            ...
            if (betteralgs) {
                s->dlgret = askhk(
                    (Frontend *)s->ppl.frontend, s->hostkey_alg->ssh_id, betteralgs,
                    ssh2_transport_dialog_callback, s);
                sfree(betteralgs);
            } else {
                s->dlgret = askalg((Frontend *)s->ppl.frontend, "host key type",
                                   s->hostkey_alg->ssh_id,
                                   ssh2_transport_dialog_callback, s);
            }
            crMaybeWaitUntilV(s->dlgret >= 0);
        ...
        if (s->warn_cscipher) {
            s->dlgret = askalg((Frontend *)s->ppl.frontend,
                               "client-to-server cipher",
                               s->out.cipher->name,
                               ssh2_transport_dialog_callback, s);
            crMaybeWaitUntilV(s->dlgret >= 0);
    ...
    if (s->kex_alg->main_type == KEXTYPE_DH) {
         ...
         */
        set_busy_status((Frontend *)s->ppl.frontend, BUSY_CPU); /* this can take a while */
        s->e = dh_create_e(s->dh_ctx, s->nbits * 2);
        pktout = ssh_bpp_new_pktout(s->ppl.bpp, s->kex_init_value);
        put_mp_ssh2(pktout, s->e);
        pq_push(s->ppl.out_pq, pktout);

        set_busy_status((Frontend *)s->ppl.frontend, BUSY_WAITING); /* wait for server */
        crMaybeWaitUntilV((pktin = ssh2_transport_pop(s)) != NULL);
        ...
        set_busy_status((Frontend *)s->ppl.frontend, BUSY_CPU); /* cogitate */
        ...
        set_busy_status((Frontend *)s->ppl.frontend, BUSY_NOT);
    ...
    if (s->gss_kex_used) {
        ...
        if (!s->got_session_id) {
            ...
            if (s->dlgret == 0) {          /* did not match */
                ...
                s->dlgret = verify_ssh_host_key((Frontend *)s->ppl.frontend,
            ...
            sfree(s->fingerprint);
            s->fingerprint = NULL;
            gStorage->store_host_key(s->savedhost, s->savedport,
     ...
     * keys.
     */
    update_specials_menu((Frontend *)s->ppl.frontend);
    ...
    struct ssh2_transport_state *s)
{
    ...
    while ((ent = (struct ssh_transient_hostkey_cache_entry *)delpos234(s->transient_hostkey_cache, 0)) != NULL) {
    ...
    struct ssh2_transport_state *s, ssh_key *key)
{
    ...
    if ((ent = (struct ssh_transient_hostkey_cache_entry *)find234(s->transient_hostkey_cache, (void *)ssh_key_alg(key),
    ...
    retd = (struct ssh_transient_hostkey_cache_entry *)add234(s->transient_hostkey_cache, ent);
    assert(retd == ent);
}
...
{
    struct ssh_transient_hostkey_cache_entry *ent =
        (struct ssh_transient_hostkey_cache_entry *)find234(s->transient_hostkey_cache, (void *)alg,
