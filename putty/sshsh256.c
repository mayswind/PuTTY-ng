...
static ssh_hash *sha256_newobj(const struct ssh_hashalg *alg)
{
...
static ssh_hash *sha256_copy(ssh_hash *hashold)
{
    ...
    ssh_hash *hashnew = sha256_newobj(hashold->vt);
...
const struct ssh_hashalg ssh_sha256 = {
    sha256_newobj, sha256_copy, sha256_final, sha256_free, 32, "SHA-256"
};
...
static ssh2_mac *hmacsha256_newobj(
                                ...
                                const unsigned char *key, int len)
{
    ...
    marshal_put_data(&ctx->sha[0], foo, 64);

    memset(foo, 0x5C, 64);
    for (i = 0; i < len && i < 64; i++)
	foo[i] ^= key[i];
    SHA256_Init(&ctx->sha[1]);
    marshal_put_data(&ctx->sha[1], foo, 64);
...
{
    struct hmacsha256 *ctx = FROMFIELD(mac, struct hmacsha256, mac);
    sha256_key_internal(ctx, (unsigned char *)key, ctx->mac.vt->keylen);
}
...
static void hmacsha256_genresult(ssh2_mac *mac, unsigned char *hmac)
{
    ...
    BinarySink_COPIED(&s);
    marshal_put_data(&s, intermediate, 32);
    SHA256_Final(&s, hmac);
}

const struct ssh2_macalg ssh_hmac_sha256 = {
    hmacsha256_newobj, hmacsha256_free, hmacsha256_key,
