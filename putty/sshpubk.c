                     ...
                     char *passphrase)
{
     ...
     */
    marshal_put_data(buf, rsa_signature, sizeof(rsa_signature));
                                       ...
                                       const char **errorstr)
{
	...
	SHA_Init(&s);
	put_uint32(&s, 0);
	marshal_put_data(&s, passphrase, passlen);
	SHA_Final(&s, key + 0);
	SHA_Init(&s);
	put_uint32(&s, 1);
	marshal_put_data(&s, passphrase, passlen);
	...
	if (is_mac) {
	    ...
	    SHA_Init(&s);
	    marshal_put_data(&s, header, sizeof(header)-1);
                    ...
                    char **commentptr, const char **errorstr)
{
    ...
    if (commentptr)
        *commentptr = comment;
        ...
        sfree(comment);
    marshal_put_data(bs, pubblob, pubbloblen);
    sfree(pubblob);
    return TRUE;
		      ...
		      char *passphrase)
{
	...
	marshal_put_data(&s, header, sizeof(header)-1);
	if (passphrase)
	    marshal_put_data(&s, passphrase, strlen(passphrase));
	SHA_Final(&s, mackey);
	hmac_sha1_simple(mackey, 20, macdata->s,
