...
typedef enum { THROTTLED, UNTHROTTLING, UNTHROTTLED } ssh2_channel_throttle_state;
struct ssh2_channel {
...
static void ssh2_connection_free(PacketProtocolLayer *ppl)
{
    ...
    while ((c = (struct ssh2_channel *)delpos234(s->channels, 0)) != NULL)
        ssh2_channel_free(c);
    freetree234(s->channels);

    if (s->x11disp)
	x11_free_display(s->x11disp);
    while ((auth = (struct X11FakeAuth *)delpos234(s->x11authtree, 0)) != NULL)
        x11_free_fake_auth(auth);
    freetree234(s->x11authtree);

    while ((rpf = (struct ssh_rportfwd *)delpos234(s->rportfwds, 0)) != NULL)
        free_rportfwd(rpf);
    ...
    while (1) {
        ...
        switch (pktin->type) {
            ...
            if (ptrlen_eq_string(type, "x11")) {
                ...
                if (!s->X11_fwd_enabled && !s->connshare) {
                    ...
                    c->chan = x11_new_channel(
                        s->ppl.frontend, s->x11authtree, &c->sc, addrstr, peerport,
                        s->connshare != NULL);
                ...
                realpf = (struct ssh_rportfwd *)find234(s->rportfwds, &pf, NULL);
          ...
          case SSH2_MSG_CHANNEL_FAILURE:
            ...
            c = (struct ssh2_channel *)find234(s->channels, &localid, ssh2_channelfind);
...
static void ssh2_setup_pty(struct ssh2_channel *c, PktIn *pktin, void *ctx)
{
    ...
    put_uint32(pktout, 0);	       /* pixel height */
    {
        ...
        write_ttymodes_to_packet_from_conf(
            BinarySink_UPCAST(modebuf), (Frontend *)cs->ppl.frontend, cs->conf,
            2, s->ospeed, s->ispeed);
                                           ...
                                           PktIn *pktin, void *ctx)
{
	...
	struct ssh_rportfwd *realpf = (struct ssh_rportfwd *)del234(s->rportfwds, rpf);
	assert(realpf == rpf);
...
static void ssh2_delete_sharing_channel(ConnectionLayer *cl, unsigned localid)
{
    struct ssh2_connection_state *s =
        FROMFIELD(cl, struct ssh2_connection_state, cl);
    struct ssh2_channel *c = (struct ssh2_channel *)find234(s->channels, &localid, ssh2_channelfind);
    if (c)
        ssh2_channel_destroy(c);
}
                         ...
                         const void *data, int length)
{
    ...
    mainchan *mc = FROMFIELD(chan, mainchan, chan);
    struct ssh2_connection_state *s = mc->connlayer;
    return from_backend((Frontend *)s->ppl.frontend, is_stderr, data, length);
}
...
static void mainchan_send_eof(Channel *chan)
{
    ...
    if (!s->mainchan_eof_sent &&
        (from_backend_eof((Frontend *)s->ppl.frontend) || s->got_pty)) {
        /*
...
static void ssh2_throttle_all_channels(ConnectionLayer *cl, int throttled)
{
    ...
    for (i = 0; NULL != (c = (struct ssh2_channel *)index234(s->channels, i)); i++)
        ssh2_channel_check_throttle(c);
}
