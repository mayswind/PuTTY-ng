...
namespace
{
    ...
    class DockView : public view::View
    {
    public:
...
public:
    ...
    HWND popup_view() { return popup_view_; }
...
void DraggedTabController::InitWindowCreatePoint()
{
    ...
    view::View* first_tab = source_tabstrip_->base_tab_at_tab_index(0);
    view::View::ConvertPointToWidget(first_tab, &first_source_tab_point_);
                                  ...
                                  const gfx::Point& screen_point)
{
    ...
    if(tabs.empty())
    {
        ...
        view::View::ConvertPointToView(NULL, attached_tabstrip_, &tab_strip_point);
    ...
    const gfx::Point& screen_point)
{
    ...
    view::View::ConvertPointToView(NULL, attached_tabstrip_, &tab_loc);
    int x =
    ...
    if(attached_tabstrip_)
    {
        ...
        if(attached_tabstrip_ != source_tabstrip_)
        {
            ...
            GetModel(source_tabstrip_)->InsertTabContentsAt(
                data->source_model_index, data->contents, 0);
        }
        else
        {
...
void DraggedTabController::CompleteDrag()
{
    ...
    else
    {
        ...
        view::Widget* widget = source_tabstrip_->GetWidget();
    ...
    const std::vector<gfx::Rect>& renderer_bounds)
{
    ...
    std::vector<view::View*> renderers;
    for(size_t i=0; i<drag_data_.size(); ++i)
    {
...
gfx::Rect DraggedTabController::GetViewScreenBounds(view::View* view) const
{
    gfx::Point view_topleft;
    view::View::ConvertPointToScreen(view, &view_topleft);
...
void DraggedTabController::BringWindowUnderMouseToFront()
{
    ...
    HWND window = dock_info_.window();
    if(!window)
    {
        HWND dragged_view = view_->GetWidget()->GetNativeView();
        dock_windows_.insert(dragged_view);
